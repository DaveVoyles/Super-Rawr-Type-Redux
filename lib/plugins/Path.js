    ...
    .defines(function(){
        ...
        ga.Path.prototype = {
            ...
            translate : function (x, y) {
                for (var i=0, len=this.segments.length; i<len; i+=2) {
                    this.segments[i  ] += x ;
                    this.segments[i+1] += y ;
                }
            },
            ...
            // scaleY default to scaleX  ( ex : scale(2) == scale(2,2) )
             ...
             * @x1      : Line ends at this x point
             * @y1      : Line ends at this y point
             * @accuracy: number of points walked on S1 to compute next point.
             *            lower # = rounder curve. Need 40+ to notice a difference
             * @Desc    : addFunctionPath(100,100, function(r) { return 10*Math.sin(2*Math.PI*r);}, 5)
             *            will move from 0,0 to 100,100 following a sinusoid.
             *            function(r) { return 10*Math.sin(2*Math.PI*r);
             **************************************************************************************/
            addFunctionPath : function(x1,y1, fn, accuracy) {
                ...
                var x0   = path[path.length-2],
                    y0   = path[path.length-1];
                var dx   = (x1-x0);
                var dy   = (y1-y0);
             ...
             * @segmentsLength[i]: contains the length of the ith segment ((2*i, 2*i+1) ; (2*i+2, 2*i+3))
            ******************************************************************************************/
