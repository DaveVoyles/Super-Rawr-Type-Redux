...
window.EntityActorEnemyBoss = EntityBaseEnemy.extend({
    ...
    *********************/
    ...
    size:              { x:  80, y: 140 },
    offset:            { x: 140, y: 110 },
    vel:               { x: -125, y:   0 }, // -25 is default speed
    health:            400,
    MaxHealth:         400,
    bHasStopped:       false,  
    bWPExposed: 	   false,
    ...
    createHealthBar: function () {
        if (this.bCanCreateHealth === true && this.bHasDrawHealth === false) {
            ...
            ig.game.spawnEntity(EntityHealthBarBoss, 0, 0, { Unit: this });
            window._SoundManager.blastoff.play();
        }
    },
    draw: function () {
        this.parent();
...
EntityStatTurret = EntityBaseEnemy.extend({
    ...
    kill: function () {
        ...
        window._SoundManager.teleport.play();
        this.parent();
    },
    spawnParticles: function () {
        var x = this.pos.x + this.size.x / 2 - EntityExplosionParticleGrey.prototype.size.x / 2;
...
EntityWeakPoint = EntityBaseEnemy.extend({
    ...
    xOffset:  	  null,
    yOffset:      null,
    health:       2, // 10 is normal
    MaxHealth:    2, //10 is normal
    canTakeDmg:   true,
    type:         ig.Entity.TYPE.B,
    checkAgainst: ig.Entity.TYPE.C,
    collides: 	  ig.Entity.COLLIDES.NEVER,
