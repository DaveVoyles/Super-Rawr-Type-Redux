...
.requires(
	'impact.game',	'impact.font',    'impact.gaPooling',    'impact.debug.debug',
	...
	'game.StarField',    'game.SoundSource',    'game.pathPatterns'
...
window.RType = ig.Game.extend({
    ...
    setGame: function () {
        ...
        ig.game.spawnEntity(EntityActorEnemySpinShip, ig.system.width / 3, ig.system.height / 2); // Debug
    },
    setUI: function(){
    ...
    // Music added from Sound Manager
    setMusic: function () {
    ...
    createGameOverTimer: function(){
        ...
        if (this.gameOverTimer && this.gameOverTimer.delta() > 0) {
                //TODO: Create a check for win / lose
                this.setGameOver();
    ...
    clearScreen: function(){
        ...
        var enemyBoss = ig.game.getEntityByName('Boss');
        // If boss exists, get rid of him too. This check is here b/c otherwise the game clear on
        // init and there is no boss present, therefore it throws an error, refuses to load
        if (enemyBoss){ enemyBoss.kill(); };
    }
});
