...
(function () {
    ...
    ga.CanvasLib.insertMainCanvas = function insertMainCanvas(_w, _h) {
        if (_w == undefined) {
            _w = document.documentElement.clientWidth & (~3);
        }
        if (_h == undefined) {
            _h = document.documentElement.clientHeight & (~3);
        }
    ...
    ga.CanvasLib.insertCanvasLayer = function insertCanvasLayer(targetCanvas, _above) {
        ...
        if (!_above) {
            parentDiv.insertBefore(newCanvas, targetCanvas);
        } else {
            var next = targetCanvas.nextSibling;
            parentDiv.insertBefore(newCanvas, next);
        }
        newCanvas.offsetLeft = targetCanvas.offsetLeft;
        newCanvas.offsetTop = targetCanvas.offsetTop;
        return newCanvas;
    };
    ...
    function AddHiddenProp(obj, name, value) {
