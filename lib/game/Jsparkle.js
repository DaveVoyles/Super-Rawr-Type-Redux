...
.defines(function () {
    ...
    ga.JSparkle = function (ParticleCtor, maxParticleCnt, _getTime) {
        if (!(maxParticleCnt >= 1)) {
            throw ('JSparkle ctor error : Particle Count must be >=1')
        }
    ...
    ga.JSparkle.prototype = {
        ...
        spawn: function spawn(cnt /*, arg1, arg2, ... */) {
            ...
            if (arguments.length > 1) {
                ...
                for (var ii = 1; ii < arguments.length; ii++, argInd++) {
                    allArgs[argInd] = arguments[ii];
                }
                // call the callback
        ...
        autoSpawn: function autoSpawn(timeInterval, particleCount /*, spArg1, spArg2, ... */) {
            ...
            var asEmitController = isFunction(particleCount) ? function (currentTime) {
        ...
        update: function update(dt, currentTime) {
            ...
            if (this._handleDeath) {
                while (true) {
                    thisParticle = particleLoopBuffer[index];
                    if (!(currentTime >= thisParticle.deathTime) && !(currentTime < thisParticle.birthTime)) { //  skip if dead or unborn)) {
                        // --------- update thisParticle  --------
                        thisParticle.update();
                    }
        ...
        draw: function draw(drawContext, currentTime) {
            ...
            while (true) {
                ...
                if (!(currentTime >= thisParticle.deathTime) && !(currentTime < thisParticle.birthTime)) {
                    // --------- draw thisParticle  --------
                    thisParticle.draw();
                }
                if (index == lastIndex) break;
        ...
        resize: function resize(newSize) {
            ...
            if (newSize < length) {
                ...
                if ((!this.currentCount) || this.currentCount && this.lastParticleIndex < newSize) {
                ...
                if (this.currentCount >= newSize) {
                    ...
                    var index = (this.firstParticleIndex + (this.currentCount - newSize)) % length,
                        i2 = 0;
                    do {
        ...
        startRunLoop: function startRunLoop(drawContext, _runLoopPreDraw, _runLoopPostDraw) {
            if (!drawContext) {
                throw ('JSparkle error : startRunLoop : drawContext cannot be null.')
            }
        ...
        _runLoop: function _runLoop() {
            ...
            var now = performance.now(),
                interval = now - this._lastRealTime;

            if (interval > 12) { // 
                if (interval > 100) {
                    interval = 16;
                } // in case window was tabbed out.
        ...
        displayStats: function displayStats(ctx, x, y, scale) {
            ...
            if (this.firstParticleIndex < this.lastParticleIndex) {
                ...
                ctx.fillRect(10 + 100 * this.firstParticleIndex / length, 160, (100 - 100 * this.firstParticleIndex / length), 8);
                ...
                if (!thisArguments) {
                    ...
                    if (thisEmiter.containsFunction) {
                        ...
                        for (var i = 0; i < thisArguments.length; i++) {
                            if (isFunction(thisArguments[i])) thisArguments[i] = thisArguments[i].call(this._particleProto)
                        }
                    }
        ...
        _measureScreenFps: function _measureScreenFps(MeasureDoneCallback) {
            ...
            var measure = function () {
                if (!startMeasureTime) {
                    startMeasureTime = performance.now();
    ...
    Object.defineProperty(Function.prototype, 'setOnPrototype', {
        value: function (o) {
            for (var p in o) {
    ...
    window.performance.now = p.now || p.webkitNow || p.msNow || p.mozNow || Date.now || function () {
        return +new Date();
    };
