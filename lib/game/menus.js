...
ig.module(
 ...
 .requires(
    ...
    window.Menu = ig.Class.extend({
        ...
        update: function () {
            if (ig.input.pressed(Bindings.up)) {
                this.current--;
            }
            if (ig.input.pressed(Bindings.down)) {
                this.current++;
            }
            this.current = this.current.limit(0, this.items.length - 1);
            if (ig.input.pressed(Bindings.left)) {
                this.items[this.current].left();
            }
            if (ig.input.pressed(Bindings.right)) {
            ...
            if (hoverItem) {
                ...
                if (ig.input.pressed(Bindings.shoot)) {
                    hoverItem.click();
                }
            } else {
                ig.system.canvas.style.cursor = 'auto';
            }
            if (ig.input.pressed(Bindings.ok)) {
